...
from onnxruntime_predict import initialize, gloves_predict_image, helmet_predict_image
...
@app.route('/glovesimage', methods=['POST'])
...
def gloves_predict_image_handler(project=None, publishedName=None):
    try:
        imageData = None
        ...
        results = gloves_predict_image(img)
...
@app.route('/<project>/detect/iterations/<publishedName>/image', methods=['POST'])
@app.route('/<project>/detect/iterations/<publishedName>/image/nostore', methods=['POST'])
def helmet_predict_image_handler(project=None, publishedName=None):
    try:
        imageData = None
        if ('imageData' in request.files):
            imageData = request.files['imageData']
        ...
        img = Image.open(imageData)
        results = helmet_predict_image(img)
        return jsonify(results)
    except Exception as e:
        print('EXCEPTION:', str(e))
        return 'Error processing image', 500
...
if __name__ == '__main__':
    ...
    # app.run(host='0.0.0.0', port=80, threaded=False, processes=4)
